...
import { ChevronUp, ChevronDown, ChevronsLeft, ChevronsRight, BarChart2, Trash2, MoreHorizontal } from 'lucide-react';
...
export default function ReceiptsAnalyticsTable({ receipts, isLoading, onDelete }) {
  ...
  const SortableHeader = ({ sortKey, children, className }) => {
    ...
    const icon = isSorted ? (sortConfig.direction === 'ascending' ? <ChevronUp className="w-3 h-3 ml-1" /> : <ChevronDown className="w-3 h-3 ml-1" />) : null;
    return (
      <TableHead onClick={() => requestSort(sortKey)} className={`cursor-pointer hover:bg-gray-100/50 dark:hover:bg-gray-800/50 ${className}`}>
    ...
    <Card className="border-0 bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl rounded-2xl">
      <CardHeader className="flex flex-row items-center justify-between p-4">
          ...
          <Link to={createPageUrl("Insights")}>
            <Button variant="ghost" size="sm" className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100">
              ...
              [...Array(PAGE_SIZE)].map((_, i) => (
                <TableRow key={i}>
                  <TableCell colSpan={5} className="p-2">
                    <div className="h-10 bg-gray-200/50 dark:bg-gray-800/50 rounded-md animate-pulse" />
                  </TableCell>
                </TableRow>
              ))
                ...
                <TableRow key={receipt.id} className="group hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                  <TableCell className="pl-6">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-gray-800 dark:text-gray-200">{receipt.merchant_name}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-gray-500 dark:text-gray-400">{formatDateSafe(receipt.transaction_date, "MMM d, yyyy")}</TableCell>
                  <TableCell>
                    <StoreType merchantName={receipt.merchant_name} />
                  </TableCell>
                  <TableCell className="font-semibold text-right text-gray-800 dark:text-gray-200">£{receipt.total_amount?.toFixed(2)}</TableCell>
